...
if ($HOST != clon00 && $HOST != clon01 && $HOST != clon02 && $HOST != clon03 && $HOST != clon04) source /apps/clas/u1/builds/DEVELOPMENT/packages/cms/onsite.cshrc
...
if ($HOST == jlabh1 || $HOST == jlabh2 || $HOST == jlaba1 || $HOST == jlaba2) then
  ...
  eval `~marki/bin/addpath.pl . $HOME/bin $HOME/bin/$OSNAME  $CERN_ROOT /usr/local/bin /bin /usr/sbin /apps/bin /site/bin /usr/openwin/bin /usr/dt/bin /usr/bin/X11 /opt/SUNWspro/bin /usr/ccs/bin /usr/ucb /usr/local/gnu/bin`
  ...
  eval `~marki/bin/addpath.pl . $HOME/bin $HOME/bin/$OSNAME  $CERN_ROOT/bin /site/bin`
endif
#
# umask
#
